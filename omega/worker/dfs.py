...
from omicron.core.timeframe import tf, FrameType
...
class AbstractStorage(ABC):
    ...
    @staticmethod
    def get_filename(_date: Union[datetime, date, AnyStr], frame_type: Union[FrameType, AnyStr],
                     prefix: AnyStr = "") -> AnyStr:
        """拼接文件名"""
        filename = []

        if isinstance(_date, str):
            filename.append(tf.int2date(_date))
        elif isinstance(_date, datetime) or isinstance(_date, date):
            filename.append(str(tf.date2int(_date)))
        else:
        ...
        if isinstance(prefix, str):
            ...
            raise TypeError("prefix must be type str")
    ...
    async def write(self, sec: AnyStr,
                    bar: Dict,
    ...
    async def read(self, sec: AnyStr,
                   _date: Union[datetime, date, AnyStr],
                   frame_type: Union[FrameType, AnyStr]) -> Dict:
        """
        Args:
            sec:  股票或基金的名称
            _date:  日期
