...
class Security:
    ...
    def ma(self, frame_type):
        if frame_type == "1d":
            arr = self.day_ma
            data = self.day_bars['close']
        elif frame_type == "30m":
            arr = self.m30_ma
            data = self.m30_bars['close']
        else:
    ...
    def curve_fit(self, ts:np.array, ):
        """
        如果len(ts) in [3, 4, 5]，则同时做线性回归和二项式回归。对多周期ts，只看是否符合线性
        回归。
    ...
    def _merge_bars(self, old: np.array, new: np.array):
        ...
        if n0['date'] == o0['date'] and n1['date'] == o1['date']:
            logger.debug("重复数据: (%s,%s), (%s,%s)", n0['date'], o0['date'],
                         n1['date'], o1['date'])
            ...
            logger.warning("数据不连续. %s, %s", self.m30_bars[-1]['date'],
                           new[0]['date'])
...
class OnePercent:
    ...
    async def reset_jobs(self):
        ...
        for sec in self.secs.keys():
            self.scheduler.add_job(self.get_bars, trigger='cron', minute='*/3',
                                   args=[sec, '30m'])
    ...
    async def get_bars(self, code: str, frame_type: str, include_unclosed=True):
        ...
        bars = jq.get_bars(code, n, frame_type, self.fields,
                           include_now=include_unclosed,
                           df=False, fq_ref_date=arrow.now().date())
        await emit.emit('/new_bars_data', {
            "code":       code,
    ...
    async def on_new_data(self, msg):
        ...
        logger.info("new bars arrived: %s, %s, %s, %s",
                    code, frame_type, bars[-1]['date'], len(bars))
